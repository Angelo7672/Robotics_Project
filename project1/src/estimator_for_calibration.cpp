...
public:
   ...
   void poseCallback(const geometry_msgs::PoseStamped::ConstPtr& gt_pose_msg, const nav_msgs::Odometry::ConstPtr& my_pose_msg,
                     const sensor_msgs::JointState::ConstPtr& wheel_state_msg, const project1::Rpm::ConstPtr& my_rpm_msg){
        ...
        double true_quat_x = gt_pose_msg->pose.orientation.x;
        double true_quat_y = gt_pose_msg->pose.orientation.y;
        double true_quat_z = gt_pose_msg->pose.orientation.z;
        double true_quat_w = gt_pose_msg->pose.orientation.w;
        double my_quat_x = my_pose_msg->pose.pose.orientation.x;
        double my_quat_y = my_pose_msg->pose.pose.orientation.y;
        double my_quat_z = my_pose_msg->pose.pose.orientation.z;
        double my_quat_w = my_pose_msg->pose.pose.orientation.w;
       ...
       ROS_INFO("Delta->theta from ground truth orientation and my calculated orientation is %f", true_theta - my_theta);
...
int main(int argc, char **argv) {
    ...
    message_filters::Subscriber <sensor_msgs::JointState> wheel_states_sub(n, "wheel_states", 1);
    message_filters::Subscriber <project1::Rpm> my_rpm_sub(n, "wheels_rpm", 1);

    typedef message_filters::sync_policies::ApproximateTime<geometry_msgs::PoseStamped, nav_msgs::Odometry, sensor_msgs::JointState, project1::Rpm> MySyncPolicy;

    message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(10), gt_pose_sub, my_pose_sub, wheel_states_sub, my_rpm_sub);
    sync.registerCallback(boost::bind(&estimator::poseCallback, &my_estimator, _1, _2, _3, _4));
